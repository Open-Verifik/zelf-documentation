...
If a **sent** OTP already exists for the same client, phone, and `phoneGateway` and was updated within about **2 minutes**, the API returns `409` with code `otp_recently_sent` and does **not** send again.

To force a new code before that window (e.g. UI “Resend”), send the same body with `"force": true`.
