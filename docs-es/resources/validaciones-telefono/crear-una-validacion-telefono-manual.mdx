...
### Reenvío / ventana de espera

Si ya existe un OTP en estado **sent** para el mismo cliente, teléfono y `phoneGateway` actualizado en los últimos ~**2 minutos**, la API responde `409` con código `otp_recently_sent` y **no** vuelve a enviar.

Para forzar un código nuevo antes de esa ventana (p. ej. “Reenviar”), envía el mismo body con `"force": true`.
